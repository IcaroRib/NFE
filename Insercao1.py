...
import MySQLdb
import datetime
def criarDicionario(xml, string):
    ...
    for elemento in xml.childNodes:
        ...
        if len(listaFilhos) > 0:
            ...
            if len(listaFilhos[0].childNodes) == 1 and xmlValor.count("<") == 2:
                ...
                dic[nome] = criarDicionario(listaFilhos[0],"    "+string)
                
#        elif len(listaFilhos) == 0:
#            dic[nome] = criarDicionario(listaFilhos[0],"    "+string)
    return dic
...
def inserirNota(xmlNota):
    ...
    con = MySQLdb.connect(host='localhost', user='root', passwd='mysql12345', db='nfe')
    c = con.cursor()
    d = con.cursor()
    e = con.cursor()
    f = con.cursor()
...
##        else: cofinsVar = None 
        ...
        if (resultadoDET[item]['prod'].has_key('NCM')): ncmTProd = str(resultadoDET[item]['prod']['NCM'])
        else: ncmTProd = None
